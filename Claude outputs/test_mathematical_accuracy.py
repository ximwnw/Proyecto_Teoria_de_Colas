...
def erlang_c(lam: float, mu: float, c: int) -> float:
    ...
    Erlang C = (a^c / c! * c / (c - a)) /
               (Σ_{n=0}^{c-1} a^n / n! + a^c / c! * c / (c - a))

    donde a = λ/μ es la intensidad de tráfico
    """
    a = lam / mu
    rho = a / c

    if rho >= 1:
        return 1.0  # Sistema inestable, Pw = 1

    # Numerador: (a^c / c!) * (c / (c - a))
    numerator = ((a ** c) / math.factorial(c)) * (c / (c - a))

    # Denominador: suma + (a^c / c!) * (c / (c - a))
...
def theoretical_Lq(lam: float, mu: float, c: int) -> float:
    """
    Calcula Lq (longitud de cola) usando la fórmula teórica.
...
def theoretical_Wq(lam: float, mu: float, c: int) -> float:
    """
    Calcula Wq (tiempo en cola) usando Little's Law.
    Wq = Lq / λ
    """
    if lam == 0:
        return 0.0
...
def theoretical_L(lam: float, mu: float, c: int) -> float:
    """
    Calcula L (clientes en sistema) usando Little's Law.
    L = Lq + λ/μ
    """
    Lq = theoretical_Lq(lam, mu, c)
...
def theoretical_W(lam: float, mu: float, c: int) -> float:
    """
    Calcula W (tiempo en sistema) usando Little's Law.
    W = L / λ
    """
    if lam == 0:
        return 0.0
...
# Importar la función del modelo actual
...
class TestMathematicalAccuracy:
    ...
    def test_P0_accuracy_erlang_formula(self):
        ...
        test_cases = [
            (10, 5, 3),      # Sistema estable simple
            (15, 10, 2),     # Diferentes parámetros
            (20, 8, 4),      # Más clientes, más servidores
            (5, 5, 2),       # Carga moderada
            (2, 3, 1),       # Un servidor
        ]
        ...
        for lam, mu, c in test_cases:
            ...
            # Tolerance: diferencia relativa < 0.01% (0.0001)
            if theoretical > 0:
                ...
                assert abs(result['P0'] - theoretical) < 1e-10
    ...
    def test_Lq_accuracy_erlang_formula(self):
        ...
        for lam, mu, c in test_cases:
            ...
            # Tolerance: diferencia relativa < 0.01%
            if theoretical > 0.001:
                relative_error = abs(result['Lq'] - theoretical) / theoretical
                assert relative_error < 0.0001, \
                    f"Lq mismatch for λ={lam}, μ={mu}, c={c}: " \
                    f"calculated={result['Lq']:.6f}, theoretical={theoretical:.6f}, " \
                    f"error={relative_error*100:.4f}%"
    ...
    def test_littles_law_L_equals_Lq_plus_lambda_over_mu(self):
        ...
        for lam, mu, c in test_cases:
            ...
            # Tolerance: diferencia relativa < 0.001%
            relative_error = abs(result['L'] - expected_L) / expected_L
            assert relative_error < 0.00001, \
                f"Little's Law (L) violated for λ={lam}, μ={mu}, c={c}: " \
                f"calculated={result['L']:.6f}, expected={expected_L:.6f}"
    ...
    def test_littles_law_W_equals_Wq_plus_one_over_mu(self):
        ...
        for lam, mu, c in test_cases:
            ...
            # W = Wq + 1/μ (en minutos)
            service_time_minutes = 60 / mu  # convertir a minutos
            expected_W = result['Wq_minutes'] + service_time_minutes

            # Tolerance: diferencia relativa < 0.001%
            relative_error = abs(result['W_minutes'] - expected_W) / expected_W
            assert relative_error < 0.00001, \
                f"Little's Law (W) violated for λ={lam}, μ={mu}, c={c}: " \
                f"calculated={result['W_minutes']:.6f}, expected={expected_W:.6f}"
    ...
    def test_utilization_rho_calculation(self):
        ...
        for lam, mu, c in test_cases:
            ...
            assert abs(result['rho'] - expected_rho) < 1e-10, \
                f"ρ mismatch for λ={lam}, μ={mu}, c={c}: " \
                f"calculated={result['rho']:.6f}, expected={expected_rho:.6f}"
    ...
    def test_stability_condition_rho_less_than_one(self):
        ...
        # Caso inestable
        result_unstable = simple_mmc_metrics(15, 5, 2)  # ρ = 15/10 = 1.5
    ...
    def test_P0_bounds_between_zero_and_one(self):
        ...
        for lam, mu, c in test_cases:
            ...
            assert 0 < result['P0'] <= 1, \
                f"P0 out of bounds for λ={lam}, μ={mu}, c={c}: {result['P0']}"
    ...
    def test_Lq_non_negative(self):
        ...
        for lam, mu, c in test_cases:
            result = simple_mmc_metrics(lam, mu, c)
            assert result['Lq'] >= 0, \
                f"Lq negative for λ={lam}, μ={mu}, c={c}: {result['Lq']}"

    def test_rho_boundary_stability(self):
        """En el límite ρ ≈ 1, el sistema se vuelve inestable."""
        # ρ = 0.9999 (justo antes del límite)
        lam = 9.999
        mu = 5
        c = 2
        rho_target = lam / (c * mu)  # ≈ 0.99995

        result = simple_mmc_metrics(lam, mu, c)

        # Debe ser estable pero con métricas muy altas
        assert result['stability'] == True
        assert result['Lq'] > 100, "Lq debe ser muy alto cerca del límite"

    def test_erlang_c_relationship(self):
        """La probabilidad de espera Pw = Erlang C."""
        # Pw = Erlang C = P0 * (λ/μ)^c / (c! * (1-ρ))
        lam, mu, c = 10, 5, 3
        rho = lam / (c * mu)

        result = simple_mmc_metrics(lam, mu, c)

        # Calcular Pw desde la fórmula
        a = lam / mu
        ...
        assert relative_error < 0.0001, \
            f"Erlang C mismatch: calculated={Pw_formula:.6f}, " \
            f"erlang={Pw_erlang:.6f}, error={relative_error*100:.4f}%"
    ...
    def test_single_server_m_m_1(self):
        ...
        # Para M/M/1: Lq = λ² / (μ(μ-λ))
        expected_Lq = (lam ** 2) / (mu * (mu - lam))

        relative_error = abs(result['Lq'] - expected_Lq) / expected_Lq
        assert relative_error < 0.0001, \
            f"M/M/1 Lq mismatch: calculated={result['Lq']:.6f}, " \
            f"expected={expected_Lq:.6f}"
    ...
    def test_metric_relationships_consistency(self):
        ...
        for lam, mu, c in test_cases:
            ...
            # Relación 2: W = Wq + 1/μ (en minutos)
            expected_W = result['Wq_minutes'] + (60 / mu)
            assert abs(result['W_minutes'] - expected_W) < 1e-6

            # Relación 3: Wq = Lq / λ (en horas convertidas a minutos)
            if lam > 0:
